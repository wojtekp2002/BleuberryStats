...
export class HarvestService {
  ...
  async addHarvest(employerId: string, employeeId: string, kg: number) {
    ...
    if (!employee) {
      throw new BadRequestException('Nie znaleziono pracownika');
    }
    // Sprawdzenie, czy dany pracownik należy do zalogowanego pracodawcy
  ...
  async getHarvests(userId: string, role: string) {
    if (role === 'EMPLOYEE') {
      return await this.harvestModel.find({ employee: userId }).sort({ date: -1 });
    } else if (role === 'EMPLOYER') {
      const employees = await this.userModel.find({ employer: userId }).select('_id');
      const ids = employees.map(e => e._id);
      return await this.harvestModel.find({ employee: { $in: ids } })
        .sort({ date: -1 })
        .populate('employee');
    }
    return [];
  ...
  async payoutOne(employerId: string, harvestId: string) {
    ...
    if (!harvest) {
      throw new BadRequestException('Nie znaleziono wpisu');
    }
  ...
  async payoutAll(employerId: string, employeeId: string) {
    ...
    await this.harvestModel.updateMany({ employee: employee._id, paidOut: false }, { paidOut: true });
